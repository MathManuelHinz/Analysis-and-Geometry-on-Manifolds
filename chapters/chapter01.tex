...
\section{Classification of topological manifolds (proofs are not examable)}
...
Let \(M_i^\circ = M_i \setminus \phi_i^{-1}(B_{\frac{1}{2}})\). Let \(M_1 \# M_2 \coloneqq M_1^\circ \perp M_2^\circ /\sim\),
where \(X\in M_1^\circ \sim y\in M_2^\circ\) if \(x\in \phi_1^{-1}(\partial \overline{B_{\frac{1}{2}}})\) and \(y=(\phi_2^{-1}\circ \phi_1) (x)\)
%sketch 1.15
\dhighlight{Fact:s}
\begin{itemize}
...
\subsection{Classification of high dimensional manifolds (not examable at all)}
...
\begin{example}
    ...
    connected \(4\)-manifold. Which is provably to hard?
\end{example}
